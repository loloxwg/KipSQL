...
impl ScalarExpression {
    pub fn eval_column(&self, batch: &RecordBatch) -> Result<ArrayRef, ExecutorError> {
        match &self {
            ScalarExpression::Constant(val) => Ok(val.to_array_of_size(batch.num_rows())),
            ScalarExpression::ColumnRef(col) => {
                // FIXME: 此处ColumnRef理应被优化器下推至TableScan中,
                // 并且无法实现eval_column，因为ColumnCatalog并无对应的index
                Ok(batch.column(0).clone())
            }
            ScalarExpression::InputRef { index, .. } => Ok(batch.column(*index).clone()),
            ScalarExpression::Alias { expr, .. } => expr.eval_column(batch),
            ScalarExpression::TypeCast { expr, ty, .. } => Ok(cast(
                &expr.eval_column(batch)?,
                &DataType::from(ty.clone()),
            )?),
            ...
            ScalarExpression::IsNull { expr } => Ok(DataValue::bool_array(
                batch.num_rows(),
                &Some(expr.nullable()),
            )),
    ...
    pub fn eval_field(&self, batch: &RecordBatch) -> Field {
        match self {
            ...
            ScalarExpression::InputRef { index, .. } => batch.schema().field(*index).clone(),
