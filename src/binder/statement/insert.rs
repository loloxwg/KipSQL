...
use crate::types::{ColumnIdT, DataType, DataTypeKind};

/// A bound `INSERT` statement.
#[derive(Debug, PartialEq, Clone)]
pub struct BoundInsert {
    pub table_ref_id: TableRefId,
    pub column_ids: Vec<ColumnIdT>,
...
impl Binder {
    pub fn bind_insert(&mut self, stmt: &Statement) -> Result<BoundInsert, BindError> {
        ...
        let col_set: HashSet<ColumnIdT> = column_ids.iter().cloned().collect();
    ...
    pub fn bind_table_columns(
        ...
        let (_, schema_name, table_name) = split_name(table_name)?;
        let schema = self
            .catalog
