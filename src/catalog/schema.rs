...
struct Inner {
    ...
    tables: HashMap<TableIdT, Arc<TableCatalog>>,
    next_table_id: TableIdT,
}

impl SchemaCatalog {
    pub(super) fn new(id: SchemaIdT, name: String) -> SchemaCatalog {
        SchemaCatalog {
            id,
    ...
    pub fn add_table(&self, name: &str) -> Result<TableIdT, CatalogError> {
    ...
    pub fn del_table(&self, id: TableIdT) {
        let mut inner = self.inner.lock().unwrap();
        let catalog = inner.tables.remove(&id).unwrap();
        inner.table_idxs.remove(&catalog.name()).unwrap();
    }

    pub fn all_tables(&self) -> HashMap<TableIdT, Arc<TableCatalog>> {
        let inner = self.inner.lock().unwrap();
        inner.tables.clone()
    }

    pub fn get_table(&self, table_id: TableIdT) -> Option<Arc<TableCatalog>> {
        let inner = self.inner.lock().unwrap();
