...
impl TableCatalog {
    pub(super) fn new(id: TableIdT, name: String) -> TableCatalog {
        TableCatalog {
            id,
    ...
    pub fn add_column(&self, name: &str, desc: ColumnDesc) -> Result<ColumnIdT, CatalogError> {
    ...
    pub fn all_columns(&self) -> BTreeMap<ColumnIdT, ColumnCatalog> {
        let inner = self.inner.lock().unwrap();
        inner.columns.clone()
    }

    pub fn get_column(&self, id: ColumnIdT) -> Option<ColumnCatalog> {
        let inner = self.inner.lock().unwrap();
