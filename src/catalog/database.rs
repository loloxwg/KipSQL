...
struct Inner {
    ...
    schemas: HashMap<SchemaIdT, Arc<SchemaCatalog>>,
...
impl DatabaseCatalog {
    ...
    pub fn add_schema(&self, name: &str) -> Result<SchemaIdT, CatalogError> {
    ...
    pub fn all_schemas(&self) -> HashMap<SchemaIdT, Arc<SchemaCatalog>> {
        let inner = self.inner.lock().unwrap();
        inner.schemas.clone()
    }

    pub fn get_schema(&self, schema_id: SchemaIdT) -> Option<Arc<SchemaCatalog>> {
        let inner = self.inner.lock().unwrap();
